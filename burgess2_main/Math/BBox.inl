...
BBoxT<T>::BBoxT( const std::vector<Vec3T<T>>& points ) {
	s32 cnt = s32(points.size());
	Eigen::Vector<T, Eigen::Dynamic> X, Y, Z;
	X.resize( cnt );
	Y.resize( cnt );
	Z.resize( cnt );
	for (s32 i = 0; i < cnt; ++i) {
	...
	Eigen::MatrixXd::Index dummy0, dummy1;
	this->min.x() = X.minCoeff( &dummy0, &dummy1 );
	this->min.y() = Y.minCoeff( &dummy0, &dummy1 );
	this->min.z() = Z.minCoeff( &dummy0, &dummy1 );
	this->max.x() = X.maxCoeff( &dummy0, &dummy1 );
	this->max.y() = Y.maxCoeff( &dummy0, &dummy1 );
	this->max.z() = Z.maxCoeff( &dummy0, &dummy1 );
}
...
Bool BBoxT<T>::testRayIntersect( const Ray3T<T>& ray ) const {
	...
	return Vec3T<T> { T( BURGESS2::F32_INFINITE ) } != intarsect; // 交差位置が無限値でなければ交差が有効だと判断する
}
...
Vec3T<T> BBoxT<T>::getRayIntersectPosition( const Ray3T<T>& ray ) const {
	Vec3T<T> result{ T( BURGESS2::F32_INFINITE ) };
	T rayParam = T( BURGESS2::F32_INFINITE );
	...
	for (u32 faceId = 1; faceId <= 6; faceId++) {
		...
		if (BURGESS2::TOLERANCE < std::abs( sDen )) {
			...
			if ((result[k] >= this->min[k] && result[k] <= this->max[k]) &&
				(result[j] >= this->min[j] && result[j] <= this->max[j])) {
				if (s < rayParam) {
					rayParam = s;
				}
			}
	...
	if (Burgess2MathFn::AlmostEqual( rayParam, T( BURGESS2::F32_INFINITE ) )) {
...
Bool BBoxT<T>::testClosest( const Vec3T<T>& position, T maxDistance ) const {
	if (this->testInside( position )) { return true; } // 検索スタート位置がボックス内であればtrueを返す
	return Vec3T<T> { T( BURGESS2::F32_INFINITE ) } != this->getClosestPosition( position, maxDistance ); // 検索位置が無限値でなければ近傍が有効だと判断する
}
...
Vec3T<T> BBoxT<T>::getClosestPosition( const Vec3T<T>& position, T maxDistance ) const {
	...
	f32 restDis = T( BURGESS2::F32_INFINITE );
	for (u8 i = 0; i < 12; i++) {
		Vec3T<T> p0;
		Vec3T<T> p1;
		Vec3T<T> p2;
		...
		Vec3T<T> closestPos = Burgess2MathFn::GetClosestPositionOnTriangle( position, p0, p1, p2 );
