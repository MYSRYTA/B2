namespace Burgess2MathFn {
	...
	std::vector<T> RandomTArray ( u32 count, u32 seed, u32 index, u32 shift, T min, T max ) {
		...
		for ( u32 i = 0; i<count; i++ ) {
			result [i] = RandomT<T>( seed * ( i+1 ), index * shift * ( i+2 ), min, max );
		}
		return result;
	}
	...
	Vec3T<T> RandomVec3 ( u32 seed, u32 index, T min, T max ) {
		return Vec3T<T> {
			RandomT<T> ( seed, index, min, max ),
			RandomT<T> ( seed, index + 15, min, max ),
			RandomT<T> ( seed, index + 48, min, max )
		};
	}
	...
	std::vector<Vec3T<T>> RandomVec3Array ( u32 count, u32 seed, u32 index, u32 shift, T min, T max ) {
		...
		for ( u32 i = 0; i<count; i++ ) {
			result [i] = RandomVec3<T> ( seed * ( i+1 ), index * shift * ( i+2 ), min, max );
		}
		return result;
	}
	...
	QuatT<T> RandomQuat ( u32 seed, u32 index, T min, T max ) {
		QuatT<T> result;
		result[0] = RandomT<T> ( seed, index, -1, 1 );
		result[1] = RandomT<T> ( seed, index + 15, -1, 1 );
		result[2] = RandomT<T> ( seed, index + 24, -1, 1 );
		result[3] = RandomT<T> ( seed, index + 8, min, max );
		result.setUnit ();
		return result;
	}
	...
	std::vector<QuatT<T>> RandomQuatArray ( u32 count, u32 seed, u32 index, u32 shift, T min, T max ) {
		...
		for ( u32 i = 0; i<count; i++ ) {
			result [i] = RandomQuat<T> ( seed * ( i+1 ), index * shift * ( i+2 ), min, max );
		}
		return result;
	}
	...
	static Deg3T<T> RandomDeg3 ( u32 seed, u32 index, T min, T max ) {
		Deg3T<T> result;
		result.x = RandomT<T> ( seed, index, min, max );
		result.y = RandomT<T> ( seed, index+15, min, max );
		result.z = RandomT<T> ( seed, index+24, min, max );
		return result;
	}
	...
	static std::vector<Deg3T<T>> RandomDeg3Array ( u32 count, u32 seed, u32 index, u32 shift, T min, T max ) {
		std::vector<QuatT<T>> result ( count );
		for ( u32 i = 0; i < count; i++ ) {
			result[i] = RandomDeg3<T>( seed * ( i + 1 ), index * shift * ( i + 2 ), min, max );
		}
		return result;
	}
}
