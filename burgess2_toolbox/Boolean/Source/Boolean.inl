...
void BooleanT<T>::CrossDataSet(
	...
	for (s32 i = 0; i < edgeList0.size (); i++) {
		...
		allLocList[i] = sq1.getAllLineIntersect ( startPos, endpos, 0, t ); // ロケーションがオンポイント/オンエッジだった場合、隣接するポリゴンからポリゴン番号が一番若いロケーションだけを返す
	}
	...
	for ( u32 i = 0; i < edgeList0.size(); i++ ) {
		...
		for ( u32 j = 0; j < locList1.size(); j++ ) {
			...
			T ratio0;
			if (startPos.almostEqual ( edgeCrossPos1, pos_tolerance )) {
				ratio0 = T ( 0.0 );
			} else if (endpos.almostEqual ( edgeCrossPos1, pos_tolerance )) {
				ratio0 = T ( 1.0 );
			} else {
			...
			///////////////////////////////////////////////////////////////////////////////////////////////
			// ロケーションデータが指し示すロケーションが頂点上 / エッジ上 / ポリゴンエリア上のどれに当たるか検査する
			...
			if ( poiIDs1.size() == 1 ) { // ロケーションがメッシュ頂点上の場合
				for ( u32 k = 0; k < edgeNeighberPoly0.size(); k++ ) {
					...
					for ( u32 l = 0; l < polyIDs.size(); l++ ) {
						crossData0.push( edgeNeighberPoly0[k], edgeCrossPos1, Vec2s{ s32( polyIDs[l] ), -2 }, edgePoiID, r );
					}
				}
...
void BooleanT<T>::MakeSplitPolygonsList(
	...
	for ( u32 j = 0; j < mesh0.polygonCount(); j++ ) {
		...
		while ( true ) {
			...
			s32 chainPoly = -10, endPoly = -1;

			// ポイントIDが正しい値か確認するために、一旦仮の変数に保存する
			s32 restIDs[2] = { -1,-1 };
			T restRatios[2];

			// オンエッジ(Vec2.y=-1)を見つけて使用済みのフラグをＯＮにする
			Bool findOnEdge = false;
			...
			// オンポイント(Vec2.y=-2)を見つけて使用済みのフラグをＯＮにする
			if ( findOnEdge == false ){
				for ( u32 k = 0; k < crossData0.crossPoly[j].size(); k++ ){
					if ( used[k] == false && crossData0.crossPoly[j][k].y() == -2 ){
						restIDs[0] = crossData0.splitEdgeSPoiID[j][k];
						restRatios[0] = crossData0.splitRatio[j][k];
						//rSplitPositions.push_back( crossPos0[j][k] );
						chainPoly = crossData0.crossPoly[j][k].x();
			...
			while ( true ) {
				...
				s32 rID = -1;
				Bool find = false;
				// まずはx/y両方にポリゴン番号の登録がある要素から判定を始める
				for ( u32 k = 0; k < crossData0.crossPoly[j].size(); k++ ) {
					if ( used[k] == true ) { continue; }
					if ( crossData0.crossPoly[j][k].x() < 0 || crossData0.crossPoly[j][k].y() < 0 ){ continue; }
					if ( chainPoly == crossData0.crossPoly[j][k].x() || chainPoly == crossData0.crossPoly[j][k].y() ) {
						rID = k;
						find = true;
						if ( 0 <= crossData0.crossPoly[j][k].x() + crossData0.crossPoly[j][k].y() - chainPoly ) {
							break;
						}
					}
				}
				// ↑対象が見つからなかったらオンエッジの要素を調べる
				if ( find == false ){
					for ( u32 k = 0; k < crossData0.crossPoly[j].size(); k++ ){
						if ( used[k] == true ){ continue; }

						if ( chainPoly == crossData0.crossPoly[j][k].x() && crossData0.crossPoly[j][k].y() == -1 ){
							rID = k;
							find = true;
							if ( 0 <= crossData0.crossPoly[j][k].x() + crossData0.crossPoly[j][k].y() - chainPoly ){
								break;
				...
				// ↑対象が見つからなかったらオンポイントの要素を調べる
				if ( find == false ){
					for ( u32 k = 0; k < crossData0.crossPoly[j].size(); k++ ){
						if ( used[k] == true ){ continue; }

						if ( chainPoly == crossData0.crossPoly[j][k].x() && crossData0.crossPoly[j][k].y() == -2 ){
							rID = k;
							if ( 0 <= crossData0.crossPoly[j][k].x() + crossData0.crossPoly[j][k].y() - chainPoly ){
								break;
							}
				...
				if ( rID != -1 ) {
					...
					if ( chainPoly == -1 || chainPoly == -2 ) {
						// 次回検索するポリゴン番号が無い場合はsplitPolygon()の終点に登録する
						restIDs[1] = crossData0.splitEdgeSPoiID[j][rID];
						restRatios[1] = crossData0.splitRatio[j][rID];
					} else {
						// 次回検索するポリゴン番号がある場合はsplitPolygon()の追加分割点となる座標を登録する
				...
				// chainPolyが-1or-2(=次回検索するポリゴン番号が無い)場合は集めた情報を登録して終了
				if ( chainPoly == -1 || chainPoly == -2 ) {
...
//------------------------------------------------------------------------------------------------------------------------------------------------
template<typename T>
Bool BooleanT<T>::core( OUT std::vector<PolygonMeshT<T>>& result, const PolygonMeshT<T>& meshA, const PolygonMeshT<T>& meshB ) {
	if (this->initalized == false || this->pointSizeA == 0 || this->pointSizeB == 0 || this->polygonSizeA == 0 || this->polygonSizeB == 0) { return false; }

	T pos_tolerance{ 0.001 };
	T ratio_tolerance{ 0.01 };

	// Boolean_CrossData
	BooleanCrossDataT<T> crossDataA ( meshA.polygonCount () );
	BooleanCrossDataT<T> crossDataB ( meshB.polygonCount () );
	this->CrossDataSet ( crossDataA, crossDataB, this->edgeListA, meshA, meshB, this->sqB, pos_tolerance );
	this->CrossDataSet ( crossDataB, crossDataA, this->edgeListB, meshB, meshA, this->sqA, pos_tolerance );
	...
	#pragma omp parallel sections
	{
		#pragma omp section
		{
			...
			result[0].splitEdgesAndPolygons ( polygonIDs, pointIDs1, ratio1, pointIDs2, ratio2, addPointPositions, ratio_tolerance, pos_tolerance );
			// 1度目のスプリットで実行ミスがあった場合
		...
		#pragma omp section
		{
			...
			result[1].splitEdgesAndPolygons ( polygonIDs, pointIDs1, ratio1, pointIDs2, ratio2, addPointPositions, ratio_tolerance, pos_tolerance );
