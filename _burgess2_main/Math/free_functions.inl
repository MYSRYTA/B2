namespace Burgess2MathFn {
	...
	Bool TestInsideTriangle ( const Vec3T<T> &P, const Vec3T<T> &triP0, const Vec3T<T> &triP1, const Vec3T<T> &triP2, T tolerance ) {
		...
		if (onEdge) {
			Vec3T<T> w = Burgess2MathFn::GetTriBarycentric ( P, triP0, triP1, triP2, tolerance, false );
			if (w[0] < T ( -BURGESS2::TOLERANCE ) || w[1] < T ( -BURGESS2::TOLERANCE ) || w[2] < T ( -BURGESS2::TOLERANCE )) {
				return false;
			}
			return true;
		}
	...
	Vec3T<T> GetClosestPositionOnTriangle ( const Vec3T<T> &P, const Vec3T<T> &triP0, const Vec3T<T> &triP1, const Vec3T<T> &triP2 ) {
		Vec3T<T> result ( BURGESS2::F32_INFINITE );
		...
		T vector3 = p0.dot ( edge0 ) * T(-1.0);
		T vector4 = p0.dot ( edge1 ) * T(-1.0);
		...
		if ( t0 + t1 <= d0 ){
			...
			if ( t0 < T(0.0) ) {
				...
				if ( rest0 < rest1 ){
					T val0 = rest1 - rest0;
					T val1 = vector0 - T(2.0) * vector1 + vector2;
					if ( val1 <= val0 ) {
						t0 = T(1.0);
						t1 = T(0.0);
					} else {
	...
	u32 GetClosestPointID ( const std::vector<Vec3T<T>> &points, const Vec3T<T> &startPosition, T maxDistance ) {
		...
		if ( loopCnt == 0 ) { return BURGESS2::U32_INFINITE; }
		...
		u32 result = Burgess2MathFn::MinimumIndex ( dist );
		return dist[result] < SQ( maxDistance ) ? result : BURGESS2::U32_INFINITE;
	}
	...
	std::vector<u32> GetPointIDsInSphere ( const std::vector<Vec3T<T>> &points, const Vec3T<T> &centerPosition, T sphereRadius, Bool sort ) {
		...
		if ( sort ) {
			...
			Burgess2MathFn::Sort ( dist2, result );
		} else {
			for ( u32 i = 0; i < loopCnt; i++ ) {
				if ( dist[i] < sphereRadiusSQ ) {
					result.push_back ( i );
	...
	Vec3T<T> GetClosestPositionOnLineSegment ( const Vec3T<T> &P, const Vec3T<T> &segmentP0, const Vec3T<T> &segmentP1, OUT T &segment_t, T tolerance ) {
		...
		if ( dot1 < T( 0.0 ) ) {
			segment_t = T( 0.0 );
		}else if ( Burgess2MathFn::AlmostEqual( dot1, T( 0.0 ), tolerance ) ){
			segment_t = T( 0.0 );
		} else {
			T dot2 = linVec.dot ( linVec );
			if ( dot2 < dot1 ) {
				segment_t = T ( 1.0 );
			} else if ( Burgess2MathFn::AlmostEqual( dot2, dot1, tolerance ) ) {
				segment_t = T ( 1.0 );
			} else {
				segment_t = dot1 / dot2;
			}
	...
	Vec3T<T> GetClosestPositionOnLineSegment ( const Vec3T<T>& P, const Vec3T<T>& segmentP0, const Vec3T<T>& segmentP1 ) {
		T segment_t;
		return GetClosestPositionOnLineSegment ( P, segmentP0, segmentP1, segment_t, BURGESS2::TOLERANCE);
	}
	...
	Bool TestRayIntersectPlane ( const Ray3T<T> &ray, const Vec3T<T> &planePoint, const Vec3T<T> &planeNormal, T tolerance ) {
		...
		//Rayがプレーンと並行の場合は接触なし
		if ( Burgess2MathFn::AlmostEqual(dot, T(0.0), tolerance ) ) {
			return false;
		}
	...
	Bool TestRayIntersectTriangle ( const Ray3T<T> &ray, const Vec3T<T> &triP0, const Vec3T<T> &triP1, const Vec3T<T> &triP2, Bool doubleSide, T tolerance ) {
		...
		T D = triP0.dot ( N );
		if ( ( doubleSide && Burgess2MathFn::AlmostEqual ( std::abs ( sDen ), T ( 0.0 ), tolerance ) == false ) ||
	...
	Vec3T<T> GetRayIntersectPositionToPlane ( const Ray3T<T> &ray, const Vec3T<T> &planePoint, const Vec3T<T> &planeNormal ) {
		Vec3T<T> delta = ray.getStartPosition() - planePoint;
		T dot = planeNormal.dot ( ray.getDirection() );
		if ( std::abs ( dot ) < BURGESS2::TOLERANCE ) {/* Rayがプレーンと並行の場合は接触なし */
			return Vec3T<T> ( T(BURGESS2::F32_INFINITE) );
		}
		T normalDot = -planeNormal.dot ( delta );
		T dist = normalDot / dot;
		if ( dist < BURGESS2::TOLERANCE ) {/* 距離が0の場合は接触なし*/
			return Vec3T<T> { T(BURGESS2::F32_INFINITE) };
		}
	...
	Vec3T<T>GetRayIntersectPositionToTriangle ( const Ray3T<T> &ray, const Vec3T<T> &triP0, const Vec3T<T> &triP1, const Vec3T<T> &triP2, u8 faceSide, T tolerance ) {
		...
		if ( side ) {
			...
			switch ( faceSide ){
				case 0:
					if ( Burgess2MathFn::AlmostEqual( T ( 0.0 ), N.dot ( N2 ), tolerance ) ){
						return Vec3T<T> { T ( BURGESS2::F32_INFINITE ) };
					}
					break;
				case 1:
			...
			return TestInsideTriangle ( pos, triP0, triP1, triP2, tolerance ) ? pos : Vec3T<T> ( T(BURGESS2::F32_INFINITE) );
		}
		return Vec3T<T> ( T(BURGESS2::F32_INFINITE) );
	}
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------
	//!	     root空間におけるチェインの先端のGlobal値を取得する
	//------------------------------------------------------------------------------------------------------------------------------------------------
	template<typename T>
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------
	//!	     チェイン状のLocal-Xfo[]をroot空間におけるGlobal値に変換する
	//------------------------------------------------------------------------------------------------------------------------------------------------
	template<typename T>
	std::vector<XfoT<T>> ChainToGlobalXfo( const XfoT<T>& rootGXfo, const std::vector<XfoT<T>>& chainLocalXfo ){
		u32 cnt = u32( chainLocalXfo.size() );
		std::vector<XfoT<T>> result( cnt );
		if ( 0 < cnt ){
			result[0] = rootGXfo * chainLocalXfo[0];
			for ( u32 i = 1; i < cnt; ++i ){
				result[i] = result[i - 1] * chainLocalXfo[i];
			}
		}
		return result;
	}
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------
	//!	     チェイン状のGlobal-Xfo[]をroot空間におけるLocal値に変換する
	//------------------------------------------------------------------------------------------------------------------------------------------------
	template<typename T>
	std::vector<XfoT<T>> ChainToLocalXfo( const XfoT<T>& rootGXfo, const std::vector<XfoT<T>>& chainGlobalXfo ){
		u32 cnt = u32( chainGlobalXfo.size() );
		std::vector<XfoT<T>> result( cnt );
		if ( 0 < cnt ){
			result[0] = rootGXfo.inverse() * chainGlobalXfo[0];
			for ( u32 i = 1; i < cnt; ++i ){
				result[i] = chainGlobalXfo[i - 1].inverse() * chainGlobalXfo[i];
			}
		}
		return result;
	}
	...
	XfoT<T> GetChainEndGlobalXfo( const XfoT<T>& rootGXfo, const std::vector<XfoT<T>>& chainLocalXfo ){
		XfoT<T> result = rootGXfo;
		for ( s32 i = 0; i < chainLocalXfo.size(); ++i ){
			result *= chainLocalXfo[i];
		}
		return result;
	}
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------
	//!	    余弦定理：３辺の長さからコサインを算出する
	//!	    @brief	余弦定理：３辺の長さからコサインを算出する
